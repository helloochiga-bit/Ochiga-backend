...
import { supabaseAdmin } from "../supabase/client"; // assumes existing
import { generateAccessCode } from "../services/codeService";
import { createQrForLink, getQrS3Url } from "../services/qrService";
...
export async function createVisitor(req: Request, res: Response) {
  try {
    ...
    // generate visitor link and QR
    const visitorId = (data as any).id;
    ...
    // optionally notify resident (share link) via existing notify service (email/whatsapp)
    try {
      await sendVisitorLinkNotification({ residentId, phone: visitorPhone, link, code: accessCode, visitorName });
    } catch (e) {
      console.warn("notify failed", e);
    }

    return res.json({ id: visitorId, link, code: accessCode, qr: qrS3Url, status: "pending", expiresAt });
  } catch (err: any) {
...
export async function getVisitorInfo(req: Request, res: Response) {
  try {
    ...
    // If approved, return routing data (basic)
    if (data.status === "approved") {
      // fetch estate map / house coordinates (stub - mapService can provide actual routing)
      const { data: mapMeta } = await supabaseAdmin.from("estate_maps").select("*").eq("estate_id", data.estate_id).limit(1).single();
    ...
    return res.json({
      ...
      rules: [
        ...
        "Follow estate rules for visitors",
...
export async function verifyVisitor(req: Request, res: Response) {
  try {
    // can accept QR or code
    const { type, value } = req.body; // type: 'code'|'qr', value: code or visitorId link
    if (!type || !value) return res.status(400).json({ error: "type and value required" });

    if (type === "code") {
      // find by code and not expired
      const { data, error } = await supabaseAdmin
        .from("visitor_access")
        .select("*")
        .eq("access_code", value)
        .in("status", ["pending", "approved"]) // still allowed
        .limit(1)
        .single();
    ...
    if (type === "qr") {
      // value expected to be visitorId or full link; extract id
      const visitorId = value.includes("/") ? value.split("/").pop() : value;
      const { data, error } = await supabaseAdmin.from("visitor_access").select("*").eq("id", visitorId).single();
...
export async function approveVisitor(req: Request, res: Response) {
  try {
    ...
    const { data, error } = await supabaseAdmin.from("visitor_access").update({
      status: "approved",
    ...
    // emit socket event to resident (if you have socket io)
    try {
      const residentId = data.resident_id;
      // import io from server? you can emit: io.to(`user:${residentId}`).emit('visitor:approved', data)
    } catch (e) { /* optional */ }
...
export async function markEntry(req: Request, res: Response) {
  try {
    ...
    // insert analytics record or update
    ...
    const { data, error } = await supabaseAdmin.from("visitor_analytics").insert([{
...
export async function markExit(req: Request, res: Response) {
  try {
    ...
    // find analytics row
    const { data: analytics } = await supabaseAdmin.from("visitor_analytics").select("*").eq("visitor_access_id", id).order("created_at", { ascending: false }).limit(1).single();

    if (!analytics) return res.status(404).json({ error: "analytics not found" });

    const exitedAt = new Date().toISOString();
    const durationMinutes = Math.round((new Date(exitedAt).getTime() - new Date(analytics.arrived_at).getTime()) / 60000);

    await supabaseAdmin.from("visitor_analytics").update({ exited_at: exitedAt, duration_minutes: durationMinutes }).eq("id", analytics.id);
...
export async function getAnalyticsForEstate(req: Request, res: Response) {
  try {
    ...
    const { data, error } = await supabaseAdmin.from("visitor_analytics").select("*").eq("estate_id", estateId).order("created_at", { ascending: false }).limit(100);
    if (error) return res.status(500).json({ error: error.message });
